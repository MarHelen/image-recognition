...
def check_circle(circle, thresh):
    """
    check if at least 70% of thresholded by 250 value sement got into consider circle
    """
    percent = 0.7
    ...
    for i in range(n):
        x = indx[1][i]
        y = indx[0][i]
        #print x,y,circle[0],circle[1]
        if check_inside_circle(x,y,circle[0],circle[1],circle[2]):
            amount += 1
            
    #print 'percent',float(amount)/n
...
def sun_detection(img):
    ...
    minRadius = 5 #1 #40
    maxRadius = 30 #5 #80
    circles = cv2.HoughCircles(thresh_2,cv2.HOUGH_GRADIENT,1,20,param1,param2,minRadius,maxRadius)
