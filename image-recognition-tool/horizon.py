...
def draw_lines(img, lines, scale):
	...
	if lines is not None and len(lines)>0:
		...
		start_y = int(lines[0][1]*scale  - (lines[0][0] * scale) * np.tan(angle))
		start = (0, start_y)

		for line in lines:
			#print 'start', start,
			cv2.line(img, start, (check_pos(line[0], img.shape[1], scale), check_pos(line[1], img.shape[0], scale)), (0, 255, 0), 5)

			if len(line) > 2:
				cv2.line(img, (check_pos(line[0], img.shape[1], scale), check_pos(line[1], img.shape[0], scale)), \
			 	(check_pos(line[2], img.shape[1], scale), check_pos(line[3], img.shape[0], scale)),(0, 255, 0), 5)

				start = (check_pos(line[2], img.shape[1], scale), check_pos(line[3], img.shape[0], scale))
			else:
				start = (check_pos(line[0], img.shape[1], scale), check_pos(line[1], img.shape[0], scale))

		#y_last = check_pos(((start[1]-lines[0][1]*scale)*img.shape[1])/start[0] + lines[0][1], img.shape[1], scale)
		y_last = ((start[1] - lines[0][1] * scale) * img.shape[1]) / start[0] + lines[0][1] * scale
...
def contrast_changing(gray_image, contrast_delta):
	#decrease contrast 
	...
	for i in range(n):
		for j in range(m):
			gray_image[i][j] = gray_image[i][j] if (gray_image[i][j] < 220) else 220
...
def find_countour(img):
	cnt = []
	#print 'img.shape', img.shape ##img.shape (197, 320)
...
def filter_contour_compare_ctg(first, second, third, arctg_delta = np.pi/60):
	"""
	compare changes between angles of triangles on bases of parallel x axis line and:
	...
	if abs(arctg_1-arctg_2) < arctg_delta: # and abs(curr_arctg) < np.pi/4:
		return 1
	else:
		return 0
		...
		while j < len(cnt) and len(max_len_subs) < (len(cnt)-j+len(curr)):
			if len(curr):
				...
				if len(curr)>2:
					...
					if (cnt[j][1] - last_added[1])*(last_added[1] - prev_added[1]) < 0 and \
					(cnt[j][1] - last_added[1]) > delta/2 and (last_added[1] - prev_added[1]) > delta/2 and \
					filter_contour_compare_ctg(first_added, prev_added, cnt[j]):
						curr[-1] = j
						unfit[j] = 0
						
					#compare angle changes from 1st added point and x-axis
					#add point if it's not changed more than on delta
					elif filter_contour_compare_ctg(first_added, last_added, cnt[j]):
						curr.append(j)
...
def line_bright_check(img, x, y):
	...
	"""
	if (y-2 > 0 and y+2 < img.shape[0]) and img[y-2][x] + 30 > img[y+2][x] and img[y-1][x] + 30 > img[y+1][x]:
		return 1
	else:
		return 0
	
	
def hybrid_add_line_check(lines, x, y, x2, y2, avg, avg_angle, gray_image, delta):
	#check if next line in delta coridor around thresh horizont line
	#check if next line start after previous
	#add observing line only if it's start point is (y_delta/2) close to prev added end point by Y
	
	len_delta = gray_image.shape[1]*0.03
	
	if lines:
		angle = calc_angle(lines[-1], (x,y) )
		#angle = np.arctan(float(y-lines[-1][1])/(x-lines[-1][0]))
		if abs(angle-avg_angle) > np.pi/30:
			return 0
	
	if ( avg - y > delta) and not lines:
		#print '3 y,avg,delta',y,avg,delta
		return 0
	if lines and abs(y-lines[-1][3]) > delta:
		return 0
	if lines and abs(y-lines[-1][3]) > delta/2 and x2-x < len_delta: # and abs(y-avg) > delta: 
		#print '1 x,y,avg,delta',x,y,avg,delta
		return 0
	if lines and x < lines[-1][2]+3:
		#print '2 x,lines[-1][2]',x,lines[-1][2]
		return 0
...
def hybrid_replace_line_check(lines, x, y, x2, y2, avg, avg_angle, image):
	...
	if abs(y-avg) > abs(lines[-1][1]-avg): # and abs(x-x2) < 2*abs(lines[-1][0]-lines[-1][2])):
	...
	if lines and y-1 > 0 and y + 1 < image.shape[0]:
		...
		if lines_y - 2 > 0 and lines_y + 1 < image.shape[0]:
			if int((image[y - 2][x] - image[y][x])) * 2 > image[lines_y - 2][lines_x] - image[lines_y][lines_x]:
				return 0
...
def hybride_build(horizon,im_shape,Hough_lines,img):
	...
	#keep those HoughLines wich start points are closest to threshold points by Y
	horizon_lines = [] #np.array([])
	n = len(horizon)
	...
	if Hough_lines is not None:
		for line in Hough_lines:
			for x,y,x2,y2 in line:
				#print 'x=', x, edge_point[0]
				if x > edge_point[0]:
					i, avg, avg_angle, edge_point = hybrid_find_y_average(horizon, x, edge_point, i, n, y_delta)
					avg = avg if avg > 0 else edge_point[1]	 
					#print 'avg', avg
				if y < (avg + y_delta) and y2 < (avg + y_delta) and \
				line_bright_check(img, x, y) and line_bright_check(img, x2, y2):

					if hybrid_add_line_check(horizon_lines, x, y, x2, y2, avg, avg_angle, img, y_delta):
						horizon_lines.append([x,y,x2,y2])
			
					elif hybrid_replace_line_check(horizon_lines, x, y, x2, y2, avg, avg_angle, img):
					#print horizont_lines
...
def build_hough_lines(canny):
	...
	threshold = 40 #40 #15  #15
	minLineLength = 20
	maxLineGap = 5
	Hough_lines = cv2.HoughLinesP(canny, rho, theta, threshold) #, minLineLength, maxLineGap)
	...
	if Hough_lines is not None:
		...
		for i in range(n):
			if Hough_lines[i][0][0] > Hough_lines[i][0][2]:
				Hough_lines[i][0][0],Hough_lines[i][0][2] = Hough_lines[i][0][2],Hough_lines[i][0][0]
				Hough_lines[i][0][1],Hough_lines[i][0][3] = Hough_lines[i][0][3],Hough_lines[i][0][1]
...
def build_threshold(gray):
	...
	thresh_val = 0
	maxValue = 255
	(thresh_param, thresh) = cv2.threshold(gray, thresh_val, maxValue,  cv2.THRESH_BINARY+cv2.THRESH_OTSU)
...
def horizon_detection(img):
	...
	#print 'hybride', hybride
	#print 'horizon_thresh ', horizon_thresh 
