...
def put_text(img, text):
    ...
    text_size, text_height = cv2.getTextSize(text, fontFace, fontScale, thickness)

    x_shift = int(text_size[0] * 1.1)
    y_shift = int(text_size[1] * 1.5)

    #frame drawing
    img_change = cv2.rectangle(img_change, (x1 ,y1), (x1 + x_shift, y1 + y_shift), darc_orange, 5, lineType=cv2.LINE_4)
    #filling drawing
    img_change = cv2.rectangle(img_change, (x1 ,y1), (x1 + x_shift, y1 + y_shift), light_orange, cv2.FILLED, lineType=cv2.LINE_4)
    ...
    cv2.putText(img_copy, text, org, fontFace, fontScale, (0,0,0), thickness)

    return img_copy


def save_results(image_name, image, image_horizon, image_sun, horizon_flag, sun_flag, daytime):
    ...
    #adjust spacing
    plt.subplots_adjust(left=None, bottom=None, right=None, top=1, wspace=1, hspace=None)
    ...
    for item in content:
        ...
        #if text present
        if item[1]:
            ax.text(image.shape[1]*2.5, image.shape[0], item[1], style='italic', bbox={'facecolor':'orange', 'alpha':0.5, 'pad':5})
    ...
    for item in content:
        ...
        if item[1]:
            ...
            ge = np.concatenate((ge,image_add), axis=0)

    plt.imshow(imutils.opencv2matplotlib(ge))
    cv2.imwrite(png_filename,ge)



    print 'check', output_file_name, 'or', png_filename, 'for the results'